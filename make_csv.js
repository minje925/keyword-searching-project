...
async function excute(keyword) {
    get_Relkeyword(keyword).then(value => {
        ...
        .then((value) => {
            ...
            if (index == 0) {
                fs.appendFile('./file/output.csv', "\uFEFF"+"키워드, 피시검색수, 모바일검색수, 총검색수,카테고리, 6개월판매량, 1일판매량, 포스팅수, 섹션순서, 1섹션점수, 2섹션점수," + "\n"
                    , function (err, sample) {
                        if (err) {
        ...
        .then(value => {
            ...
            fs.appendFile('./file/output.csv', "\uFEFF" + array[index]['title'] + "," + array[index]['pccnt']
                + "," + array[index]['mocnt']
