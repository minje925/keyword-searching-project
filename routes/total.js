...
router.get('/', (req, res) => {
    const TotalPromise = new Promise((resolve, reject)=>{
            ...
            request.get(options, function (error, response, body) {
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    for(var i = 0; i < data['items'].length; i++) {
                        ...
                        total_data.push([text, data['items'][i]['brand'], data['items'][i]['maker'], data['items'][i]['lprice'], data['items'][i]['hprice'], cate[i], data['items'][i]['image'], data['items'][i]['mallName'], i+1])
        ...
        // 숫자에 3자리수 마다 ',' 찍기
        value[1].push(value[1][1]-value[1][2]);
        value[1].push(value[1][2]-value[1][1]);
        ...
        res.render('total.ejs', {data:value});
    });
});
